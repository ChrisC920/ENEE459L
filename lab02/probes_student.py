...
def probe_torch(env: Env) -> dict[str, Any]:
    ...
    if cuda_available is True:
        ...
        out["diagnosis"] = (
            "this is an NVIDIA build but it cannot see the GPU — the wheel is "
            "right, so look at the driver stack, the container, or the user's "
            "groups, not at pip"
        )
    else:
        out["diagnosis"] = (
...
def probe_tensorrt(env: Env) -> dict[str, Any]:
    ...
    try:
        ...
        if env.python.prefix and env.python.prefix != env.python.base_prefix:
            hint = (
                " — you are inside a virtual environment, and TensorRT is a "
                "system package that a venv made without --system-site-packages "
                "cannot see"
            )
